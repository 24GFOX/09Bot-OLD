  ...
  execute(message, args, client, firebase) {
    ...
    if (!user) {
      const missingArgument_User = new Discord.MessageEmbed()
        ...
        .setDescription(
          `You're a missing a required argument (banUser) in your message. The command can't continue without this, please add it and try again`
        )
        .setColor(0xff0000)
        .setFooter(
    ...
    user
      .ban({
        reason: `User has been banned by Omega-9 helper. Command run by ${msg.author.username}`
      })
